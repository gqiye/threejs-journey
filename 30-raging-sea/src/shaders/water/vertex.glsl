...
void main()
{
    ...
    // 
    // 波浪 
    // 使用一个elevation的变量而不是直接去更新y属性，在后面为波浪着色的时候将派上用场：
    // 波浪高度只在x轴上进行改变 sin(modelPosition.x * uBigWavesFrequency.x)
    // y值在z轴上控制波浪。 sin(modelPosition.z * uBigWavesFrequency.y)
    // 波浪高度  uBigWavesElevation;
    ...
    // 大海中更汹涌的洋流时，它们更加混乱，频率各不同并且毫无规律可循。
